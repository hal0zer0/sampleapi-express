...
function sendResource(req, res, data) {
    // 'send' and 'format' are separate because sending collections also
    // uses the format function but requires different outer formatting
    data = formatResource(req, data)

    // ISO8601 formatting for timestamps
    var d = new Date()
